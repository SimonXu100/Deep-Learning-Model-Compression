...
parser = argparse.ArgumentParser(
    description='PyTorch Resnet50 Prune & Retraining')
...
if __name__ == "__main__":
    ...
    for i in range(prune_iteration):
        ...
        torch.save(model, "resnet50_chkpt/model_conv_frac_%.2f.pth"%frac_list[-1])

    result = np.vstack((frac_list, test_accus_prune, train_accus_prune, test_accus_prune_finetuned, train_accus_prune_finetuned))
    np.savetxt("resnet50_structured_performance.txt", result)
